...
Future<int?> getMemoryScoreUser(int userId) async {
  try {
    final response = await http.get(
      Uri.parse('https://s3-4677.nuage-peda.fr/api_playgames/public/api/memories?page=1&user=$userId'),
...
Future<void> majMemoryUserScore(int userId, int nouveauScore) async {
  try {
    final response = await http.patch(
      Uri.parse('https://s3-4677.nuage-peda.fr/api_playgames/public/api/memories/$userId'),
      headers: {'Content-Type': 'application/json'},
...
Future<Map<String, dynamic>> getPointMemory() async {
  final response = await http.get(
    Uri.parse('https://s3-4677.nuage-peda.fr/api_playgames/public/api/memories?page=1'),
    headers: {'Content-Type': 'application/ld+json',},
  );
