...
Future<Map<String, dynamic>> getUser(String email) async {
  final response = await http.get(
    Uri.parse('http://s3-4677.nuage-peda.fr/playgames/api/users?page=1&email=$email'),
...
Future<List<Map<String, dynamic>>> getListUsers() async {
  final response = await http.get(
    Uri.parse('http://s3-4677.nuage-peda.fr/playgames/api/users?page=1&order%5Bpoints%5D=desc'),
    headers: {'Content-Type': 'application/ld+json',},
  );
