...
Future<int?> getUserScore(int userId) async {
  try {
    final response = await http.get(
      Uri.parse('https://s3-4677.nuage-peda.fr/api_playgames/public//api/pendus?page=1&user=$userId'),
...
Future<void> createUserScore(int userId) async {
  try {
    final response = await http.post(
      Uri.parse('https://s3-4677.nuage-peda.fr/api_playgames/public//api/pendus'),
...
Future<Map<String, dynamic>> getPointPendu() async {
  final response = await http.get(
    Uri.parse('https://s3-4677.nuage-peda.fr/api_playgames/public//api/pendus?page=1'),
    headers: {'Content-Type': 'application/ld+json',},
  );
